...
export const checkOrderExistsQuery =
  'select ID as orderId ' +
  `from ${schema}.[dbo].[ECommerceOrders] ` +
  'where paymentToken = @paymentToken';

export const checkTrimbleUserExistsQuery =
  'SELECT users.UserId, users.OrganizationId, users.PrimaryLogin, users.Email, users.AdditionalEmail, users.FirstName, users.LastName ' +
...
export const checkTrimbleEmailExistsQuery =
  'SELECT count(*) as emailExists ' +
  `FROM ${schema}.[dbo].[Users] users ` +
  'WHERE email = @email and Deleted = 0';

export const checkTrimbleOrgExistsQuery =
  'SELECT org.ShortName, org.OrganizationId ' +
  `FROM ${schema}.[dbo].[Organizations] org ` +
  'WHERE upper(org.ShortName) = upper( @orgName ) ' +
  'and org.Deleted = 0';

export const checkTrimbleLoginExistsQuery =
  'SELECT count(*) as loginExists ' +
...
export const checkTrimbleLoginExistsByOrgNameQuery =
  'SELECT count(*) as loginExists ' +
  `FROM ${schema}.[dbo].[Logins] logins ` +
  ...
  'and logins.Deleted = 0';
export const selectPrimaryLoginByUserIdQuery =
  'SELECT users.PrimaryLogin as loginId ' +
  `FROM ${schema}.[dbo].[Users] users ` +
  'WHERE users.UserId = @userId';

export const getSubscriptionsByEmailQuery =
  'SELECT ' +
...
export const getTrimbleProfileByEmailQuery =
  'SELECT ' +
...
export const createTrimbleOrgQuery =
  'INSERT INTO ' +
...
export const insertRoleGroupQuery =
  'INSERT INTO ' +
...
export const updateTrimbleUserQuery =
  'UPDATE ' +
  `${schema}.[dbo].[Users] ` +
...
export const insertTrimbleSubscriptionQuery =
  'INSERT INTO ' +
  `${schema}.[dbo].[Subscriptions] ` +
...
export const getRoleGroupsQuery =
  'select roleGroups.RoleGroup ' +
  `from ${schema}.[dbo].[Users] users ` +
